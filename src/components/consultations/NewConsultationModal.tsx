...
export default function NewConsultationModal({ clients }: { clients: any[] }) {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '1rem' }}>
      <div className="glass" style={{ padding: '2rem', width: '100%', maxWidth: '500px', position: 'relative' }}>
        ...
        <h2 style={{ marginTop: 0, color: 'var(--primary-light)' }}>Schedule Consultation</h2>
        ...
        <form action={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1.5rem' }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem' }}>Type of Reading</label>
          ...
          <div>
            ...
            <textarea name="notes" className="input" style={{ width: '100%', minHeight: '80px' }}></textarea>
          </div>
          <button type="submit" className="btn-primary" style={{ marginTop: '1rem' }}>Schedule</button>
        </form>
      </div>
    </div>
  );
